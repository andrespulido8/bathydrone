...
def main():
    ...
    lambd2 = force_x_N
    H_x2 = speed_x_mps.reshape((n, 1))
    drag_x2 = np.linalg.lstsq(H_x2, lambd2, rcond=None)[0]
    print("Drag coefficient using only linear term: ", drag_x2)

    drag_x = np.linalg.lstsq(H_x, lambd, rcond=None)[0]
    print("Drag coefficient using linear and nonlinear term: ", drag_x)

    F = H_x2 @ drag_x2
    plt.plot(speed_x_mps, force_x_N, "o", label="Original data", markersize=10)
    plt.plot(speed_x_mps, F, "b", label="Fitted line")
