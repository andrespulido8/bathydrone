...
Iz = 4  # mass moment of inertia from SolidWorks
    ...
    def step(self, s, v_dr, dt, delta):
        ...
        D = np.array(
            [
                ...
                [
                    0,
                    -(d_yvv * abs(q[4]) + d_yrv * abs(q[5])),
                    -(d_yvr * abs(q[4]) + d_yrr * abs(q[5])),
                ],
                [
                    0,
                    -(d_nvv * abs(q[4]) + d_nrv * abs(q[5])),
                    -(d_nvr * abs(q[4]) + d_nrr * abs(q[5])),
                ],
            ]
        )
